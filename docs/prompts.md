...
> 소스: `argos/routers/api.py`
> 총 7개 프롬프트 + 2개 Q&A 정책 텍스트

---

## 목차

| # | 프롬프트 | 용도 | 호출 엔드포인트 | 입력 변수 | 출력 형식 | temperature |
|---|---------|------|----------------|----------|----------|-------------|
| 1 | `MANUALIZE_PROMPT` | 원문 → 구조화 매뉴얼 JSON 변환 | `POST /doc/{id}/manualize` | `raw_text` | JSON | 0.3 |
| 2 | `GATE_CHECK_PROMPT` | 섹션 품질/리스크 판정 | `POST /doc/{id}/sections/{sid}/gate`, `POST /doc/{id}/quality-gate` | `section_text`, `raw_text` | JSON | 0.2 |
| 3 | `FILL_PROMPT` | 섹션 맥락 보강 (① 단계) | `POST /doc/{id}/refine-text` (task=fill) | `section_text`, `raw_text`, `qa_policy_text` | plain text | 0.3 |
| 4 | `FINALIZE_PROMPT` | RAG 검색 최적화 (② 단계) | `POST /doc/{id}/refine-text` (task=refine) | `section_text`, `raw_text` | plain text | 0.3 |
| 5 | `CHAT_PROMPT` | RAG 챗봇 응답 생성 | `POST /chat` | `context`, `question` | JSON | 0.3 |
| 6 | `PATCH_PROMPT` | 개선 제안 → FAQ 패치 생성 | `POST /improvements/{id}/apply` | `title`, `reason`, `section_name`, `section_text` | plain text | 0.3 |
| 7 | `API_SPEC_PROMPT` | 매뉴얼에서 API 스펙 추출 | `POST /doc/{id}/extract-api-spec` | `sections_text`, `api_issues` | JSON 배열 | 0.3 |
...
- **위치**: `api.py:206`
- **역할**: 원문(raw_text)을 RAG에 적합한 구조화 매뉴얼 JSON으로 변환
- **입력 변수**: `{raw_text}` (최대 8000자 잘림)
- **출력**: RFC8259 JSON (코드블록/설명 금지)

### 프롬프트 전문

```
당신은 영업/운영 문서를 **RAG(검색 기반 답변)**에 넣기 적합한 **구조화 매뉴얼 데이터**로 변환하는 전문가입니다.
...

{
  ...
  "change_summary": "이번 변환에서 수행한 작업 요약"
}
...
- **위치**: `api.py:530`
- **역할**: 매뉴얼 섹션의 품질/리스크를 PASS / NEED_FIX / BLOCK으로 판정
- **입력 변수**: `{section_text}` (최대 4000자), `{raw_text}` (최대 4000자)
- **출력**: RFC8259 JSON
- **규칙 기반 사전 검사**: PII 패턴(주민번호, 전화번호, 이메일), 모호 표현 6종이 LLM 호출 전에 먼저 체크됨

### 프롬프트 전문

```
당신은 RAG 반영 전, 매뉴얼 섹션 텍스트(section_text)의 품질/리스크를 판정하는 QA 게이트입니다.
...
## 3. FILL_PROMPT (맥락 보강 — ① 단계)

- **위치**: `api.py:831`
- **역할**: 섹션 텍스트를 원문(raw_text) 기반으로 독립적으로 이해 가능한 수준으로 보강
- **입력 변수**: `{section_text}`, `{raw_text}` (최대 4000자), `{qa_policy_text}`
- **출력**: plain text (개선된 section_text 전체)

### Q&A 정책 (삽입 변수: `{qa_policy_text}`)

| 변수명 | 값 | 조건 |
|--------|---|------|
| `QA_POLICY_DENY` | "Q&A는 새로 추가하지 마세요. 기존 Q&A만 유지/정리하세요." | `allow_qa=false` (기본값) |
| `QA_POLICY_ALLOW` | "원문 근거가 명확한 경우에만 Q&A를 1~3개 추가할 수 있습니다. 답이 불명확하면 Q&A를 만들지 말고 [확인 필요]로 처리하세요." | `allow_qa=true` |

### 프롬프트 전문

```
당신은 RAG 청크(섹션 텍스트)를 '독립적으로 이해 가능한 매뉴얼'로 보강하는 편집자입니다.
...
- 항목 제목: "### "
- 본문: "-" bullet
- 기존 Q&A는 유지하되, 신규 Q&A는 위 Q&A 정책을 따르세요. 형식:
  - Q: ...
  - A: ...
...
## 4. FINALIZE_PROMPT (RAG 최적화 — ② 단계)

- **위치**: `api.py:867`
- **역할**: 섹션 텍스트의 검색 적중률/답변 일관성을 높이기 위한 표현/구조 최적화
- **입력 변수**: `{section_text}`, `{raw_text}` (최대 4000자)
- **출력**: plain text (최종 section_text 전체)

### 프롬프트 전문

```
당신은 RAG 검색 적중률과 답변 일관성을 높이기 위해 section_text(plain text)를 '최종 문구'로 다듬는 편집자입니다.
...
## 5. CHAT_PROMPT (RAG 챗봇)

- **위치**: `api.py:1075`
- **역할**: RAG 검색 결과(context)를 바탕으로 사용자 질문에 JSON 형식으로 응답
- **입력 변수**: `{context}` (top-5 청크), `{question}` (사용자 메시지)
- **출력**: JSON

### 프롬프트 전문

```
당신은 아파트 커뮤니티 규정에 대해 답변하는 AI 어시스턴트입니다.
...
## 6. PATCH_PROMPT (개선 제안 FAQ 패치)

- **위치**: `api.py:1295`
- **역할**: 개선 제안(improvement suggestion)을 기반으로 섹션에 추가할 FAQ 생성
- **입력 변수**: `{title}`, `{reason}`, `{section_name}`, `{section_text}` (최대 2000자)
- **출력**: plain text (---FAQ--- 형식)

### 프롬프트 전문

```
문서 섹션에 추가할 FAQ 항목을 생성하세요.
...
## 7. API_SPEC_PROMPT (API 스펙 추출)

- **위치**: `api.py:1392`
- **역할**: 매뉴얼 섹션에서 시스템 연동이 필요한 intent를 추출하여 API 스펙 생성
- **입력 변수**: `{sections_text}` (최대 4000자), `{api_issues}` (API_NEEDED 이슈 목록)
- **출력**: JSON 배열

### 프롬프트 전문

```
아래 매뉴얼 섹션에서 시스템 API가 필요한 의도(intent)를 추출하세요.
...
```
Upload → Extract → Manualize(1) → [Editor]
                                      │
                            ┌─────────┼─────────┐
                            ▼         ▼         ▼
                        Fill(3)   Finalize(4)  Gate(2)
                            │         │         │
                            └────┬────┘         │
                                 ▼              │
                         Compare Modal ─────────┘
...
1. **Hallucination 금지**: 원문에 없는 정보 생성/추측/창작 금지
2. **PII 마스킹**: 개인정보는 반드시 `***` 마스킹 유지
3. **출력 형식 준수**: JSON 프롬프트는 RFC8259 유효 JSON만, plain text 프롬프트는 텍스트만
4. **코드블록/설명 금지**: 모든 프롬프트에서 부가 설명 없이 결과만 출력
