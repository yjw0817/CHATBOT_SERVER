        ...
        if not result:
            ...
            if results:
                ...
                result = {
                    ...
                    "section_text": item["section_text"][:20000],
                    "evidence_spans": [],
                }
...
def _gate_section_internal(doc_id: str, section_name: str, cursor, raw_text_fallback: str = None):
    ...
    if sec["source_chunk_id"]:
        ...
        if chunk_row and chunk_row["raw_chunk"]:
            raw_reference = chunk_row["raw_chunk"][:30000]

    if not raw_reference:
        if raw_text_fallback is not None:
            raw_reference = raw_text_fallback[:30000]
        else:
            cursor.execute("SELECT raw_text FROM documents WHERE doc_id = %s", (doc_id,))
            doc = cursor.fetchone()
            raw_reference = (doc["raw_text"] or "")[:30000] if doc else ""
...
def fill_all(doc_id: str):
    ...
    raw_text_safe = raw_text[:30000] if raw_text else "(원본 문서를 찾을 수 없습니다. 기존 텍스트만 참고하세요.)"
...
def refine_all(doc_id: str):
    ...
    if not doc:
        ...
        raise HTTPException(status_code=404, detail="Document not found")
    raw_text = doc["raw_text"] or ""
    raw_text_safe = raw_text[:30000] if raw_text else "(원본 없음)"
...
def _manualize_batch(batch: dict, doc_id: str) -> list:
    ...
    for it in items:
        fallback.append({
            ...
            "section_text": it["section_text"][:20000],
            "evidence_spans": [],
        })
    return fallback
...
def _manualize_chunk(chunk: dict, doc_id: str) -> dict:
    ...
    for attempt in range(2):
        try:
            content = call_llm(
                MANUALIZE_CHUNK_PROMPT.format(raw_chunk=raw_chunk[:60000]),
                temperature=0.3
            )
            if content:
                json_match = re.search(r'\{[\s\S]*\}', content)
